...
function shortMessageForApiLog(full: string): string {
  const t = full.trim();
  if (t.includes("INFO-100")) return "INFO-100 등급코드 그리드 인증 실패 (포털 권한·DB 저장 키)";
...
async function fetchGrdCodesFromMafra(apiKey: string, requestId: string) {
  ...
  const collected: MafraGrdCode[] = [];
  let start = 1;
  let totalCount = Number.POSITIVE_INFINITY;

  while (start <= totalCount) {
    const end = start + PAGE_SIZE - 1;
    const url = `${BASE_HOST}/${encodeURIComponent(apiKey)}/json/${GRD_CODE_API_URL}/${start}/${end}`;
    ...
    if (!gridOk) {
      ...
      const authHint =
        payload.code === "INFO-100"
          ? " 품목·단위 동기화는 되어도 등급 그리드는 포털에서 별도 승인일 수 있습니다. data.garak.co.kr에서 「농축수산물 등급 코드」 사용 여부를 확인하세요."
          : "";
      return buildError({
        source: SOURCE,
        requestId,
        ...
        message: apiMsg ? `${apiMsg}.${authHint}` : `등급코드 API 오류.${authHint}`,
      });
    }
  ...
  return buildSuccess<MafraGrdCode[]>({
    ...
    startedAt,
    data: unique,
    message: `등급코드 동기화 성공 (${unique.length}건)`,
  });
}
...
/**
 * 데스크에서 등급 CODEID 전체 목록을 드롭다운으로 고르게 할 때 사용합니다.
 */
export async function listMafraGrdCodes(params: {
  requestId: string;
  appId?: string;
  forceSync?: boolean;
}): Promise<ApiResponse<MafraGrdCodeListResponseData>> {
  const startedAt = performance.now();
  ...
  if (needsRefresh) {
    const synced = await syncMafraGrdCodes({
      requestId: `${params.requestId}-sync`,
      appId: params.appId ?? "desk-mafra-grd-codes-list",
    });
    if (!synced.ok) {
      if (cache.items.length > 0) {
        ...
        const staleResponse = buildSuccess<MafraGrdCodeListResponseData>({
          source: SOURCE,
          requestId: params.requestId,
          startedAt,
          message: `최신 동기화에 실패했습니다. 이전에 저장된 등급 ${staleItems.length}건을 표시합니다. (${synced.message})`,
          data: {
            updatedAt: cache.updatedAt,
      ...
      return synced as ApiResponse<MafraGrdCodeListResponseData>;
    }
    cache = await loadMafraGrdCodeCache();
  }
  ...
  const response = buildSuccess<MafraGrdCodeListResponseData>({
    source: SOURCE,
    requestId: params.requestId,
    startedAt,
    message: `등급코드 ${items.length}건`,
    data: {
      updatedAt: cache.updatedAt,
      total: items.length,
      items,
