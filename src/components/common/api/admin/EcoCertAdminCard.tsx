...
import type { EcoCertSettingsOverview } from "@/components/common/api/server/admin/providerSettings";
...
export function EcoCertAdminCard({
  initialOverview,
  className,
  onOverviewChange,
}: {
  initialOverview: EcoCertSettingsOverview;
  className?: string;
  onOverviewChange?: (overview: EcoCertSettingsOverview) => void;
}) {
  ...
  async function loadOverview() {
    ...
    if (!body.ok || !body.data) {
      throw new Error(body.message ?? "친환경 인증정보 설정을 불러오지 못했습니다.");
    }
  ...
  async function saveSecret() {
    ...
    setNotice("");
    try {
      const res = await fetch("/api/admin/connectors/eco-cert/secret", {
        method: "POST",
  ...
  async function saveConfig() {
    ...
    try {
      ...
      const body = await readApiEnvelope<EcoCertSettingsOverview>(res);
      if (!body.ok || !body.data) {
        throw new Error(body.message ?? "친환경 인증정보 설정 저장에 실패했습니다.");
      }
      setOverview(body.data);
      onOverviewChange?.(body.data);
      setSaveConfigState("done");
      setNotice("친환경 인증정보 설정을 저장했습니다.");
    } catch (error) {
      setSaveConfigState("error");
      setNotice(error instanceof Error ? error.message : "친환경 인증정보 설정 저장에 실패했습니다.");
    }
  }
  ...
  async function runHealthCheck() {
    ...
    setNotice("");
    try {
      const res = await fetch("/api/admin/connectors/eco-cert-health", { method: "POST" });
      const body = await readApiEnvelope<{ totalCount: number }>(res);
      if (body.ok && body.data) {
        setHealthState("ok");
        setHealthDetail(`연결 확인 성공 (조회 결과 ${body.data.totalCount}건).`);
      } else {
        setHealthState("error");
        setHealthDetail(body.message ?? "친환경 인증정보 API 연결 확인에 실패했습니다.");
      }
      await loadOverview();
    } catch (error) {
      setHealthState("error");
  ...
  async function runSampleQuery() {
    setNotice("");
    try {
      const res = await fetch("/api/admin/connectors/eco-cert/certs/list", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const body = await readApiEnvelope<EcoCertListResponseData>(res);
      if (!body.ok || !body.data) {
        throw new Error(body.message ?? "샘플 조회에 실패했습니다.");
      }
      setLastResultCount(body.data.rows.length);
      setNotice(`샘플 조회 성공: ${body.data.rows.length}건 (총 ${body.data.totalCount}건)`);
      await loadOverview();
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "샘플 조회에 실패했습니다.");
    }
  }
      ...
      <div className="space-y-5">
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-white">친환경 인증정보 API 연결 상태</p>
              <p className="mt-1 text-xs text-zinc-400">인증 정보 목록 조회 1회로 작동 여부를 확인합니다.</p>
            </div>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-zinc-400">API URL</span>
              <input
                value={config.apiUrl}
                onChange={(event) =>
                  setConfig((current) => ({ ...current, apiUrl: event.target.value.trim() }))
                }
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-zinc-400">TYPE</span>
              <select
                value={config.type}
                onChange={(event) =>
                  setConfig((current) => ({ ...current, type: event.target.value === "XML" ? "XML" : "JSON" }))
                }
                className="h-11 w-full rounded-xl border border-white/10 bg-zinc-950/80 px-4 text-sm text-white outline-none"
              >
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => void runSampleQuery()}
              className="h-11 rounded-xl border border-white/10 bg-white/5 px-4 text-sm font-semibold text-zinc-200"
            >
              샘플 조회
            </button>
          </div>
          <p className="mt-3 text-xs text-zinc-500">
            최근 샘플 조회 건수: {lastResultCount ?? "-"}
          </p>
        </section>
