...
import type { EcoPriceSettingsOverview } from "@/components/common/api/server/admin/providerSettings";
...
export function EcoPriceAdminCard({
  initialOverview,
  className,
  onOverviewChange,
}: {
  initialOverview: EcoPriceSettingsOverview;
  className?: string;
  onOverviewChange?: (overview: EcoPriceSettingsOverview) => void;
}) {
  ...
  async function loadOverview() {
    ...
    if (!body.ok || !body.data) {
      throw new Error(body.message ?? "친환경 가격 API 설정을 불러오지 못했습니다.");
    }
  ...
  async function saveSecret() {
    ...
    setNotice("");
    try {
      const res = await fetch("/api/admin/connectors/eco-price/secret", {
        method: "POST",
  ...
  async function saveConfig() {
    ...
    try {
      ...
      const body = await readApiEnvelope<EcoPriceSettingsOverview>(res);
      if (!body.ok || !body.data) {
        throw new Error(body.message ?? "친환경 가격 API 설정 저장에 실패했습니다.");
      }
      setOverview(body.data);
      onOverviewChange?.(body.data);
      setSaveConfigState("done");
      setNotice("친환경 가격 API 설정을 저장했습니다.");
    } catch (error) {
      setSaveConfigState("error");
      setNotice(error instanceof Error ? error.message : "친환경 가격 API 설정 저장에 실패했습니다.");
    }
  }
  ...
  async function runHealthCheck() {
    ...
    setNotice("");
    try {
      const res = await fetch("/api/admin/connectors/eco-price-health", { method: "POST" });
      ...
      if (body.ok && body.data) {
        ...
        setHealthDetail(body.message ?? "친환경 가격 API 연결 확인에 실패했습니다.");
      }
      await loadOverview();
    } catch (error) {
      setHealthState("error");
      ...
      <div className="space-y-5">
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-white">친환경 가격 API 연결 상태</p>
              <p className="mt-1 text-xs text-zinc-400">목록 조회 1회로 작동 여부를 확인합니다.</p>
            </div>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-zinc-400">기본 시작일(YYYYMMDD)</span>
              <input
                value={config.fromDate}
                onChange={(event) =>
                  setConfig((current) => ({ ...current, fromDate: event.target.value.replace(/\D/g, "").slice(0, 8) }))
                }
                className="h-11 w-full rounded-xl border border-white/10 bg-zinc-950/80 px-4 text-sm text-white outline-none"
              />
            </label>
            <label className="block">
              <span className="mb-1 block text-xs font-semibold text-zinc-400">기본 종료일(YYYYMMDD)</span>
              <input
                value={config.toDate}
                onChange={(event) =>
                  setConfig((current) => ({ ...current, toDate: event.target.value.replace(/\D/g, "").slice(0, 8) }))
                }
                className="h-11 w-full rounded-xl border border-white/10 bg-zinc-950/80 px-4 text-sm text-white outline-none"
              />
            </label>
