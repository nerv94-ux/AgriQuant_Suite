...
import { getMafraApiKey, saveMafraApiKey } from "@/components/common/api/server/admin/mafraItemCodeStore";
...
export default async function handler(
  ...
  if (!apiKey) {
    return res.status(400).json(
      buildError({
        message: "MAFRA API Key를 입력해 주세요.",
        source: "SYSTEM",
        requestId,
        startedAt,
        errorCategory: ApiErrorCategory.VALIDATION_ERROR,
  ...
  await saveMafraApiKey(apiKey, session.user.email ?? null);
