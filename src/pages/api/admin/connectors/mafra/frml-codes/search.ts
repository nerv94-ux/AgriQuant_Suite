...
import type { MafraFrmlCodeSearchResponseData } from "@/components/common/api/server/connectors/mafra-frml-code";

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<MafraFrmlCodeSearchResponseData>>
) {
  if (req.method !== "GET") {
  ...
  const result = await searchMafraFrmlCodes({
    requestId: crypto.randomUUID(),
    appId: "admin-mafra-frml-codes-search",
    query,
    forceSync,
  });
