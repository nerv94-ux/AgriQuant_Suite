...
import type { MafraItemCodeSearchResponseData } from "@/components/common/api/server/connectors/mafra-item-code";

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<MafraItemCodeSearchResponseData>>
) {
  if (req.method !== "GET") {
  ...
  const result = await searchMafraItemCodes({
    requestId: crypto.randomUUID(),
    appId: "admin-mafra-item-codes-search",
    query,
    forceSync,
  });
