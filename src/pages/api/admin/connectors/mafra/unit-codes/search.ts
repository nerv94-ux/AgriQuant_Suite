...
import type { MafraUnitCodeSearchResponseData } from "@/components/common/api/server/connectors/mafra-unit-code";

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<MafraUnitCodeSearchResponseData>>
) {
  if (req.method !== "GET") {
  ...
  const result = await searchMafraUnitCodes({
    requestId: crypto.randomUUID(),
    appId: "admin-mafra-unit-codes-search",
    query,
    forceSync,
  });
