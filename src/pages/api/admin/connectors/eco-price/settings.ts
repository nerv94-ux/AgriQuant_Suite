...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ApiResponse<EcoPriceSettingsOverview>>
) {
  ...
  if (req.method === "GET") {
    const data = await getEcoPriceSettingsOverview();
  ...
  if ((fromDate && !/^\d{8}$/.test(fromDate)) || (toDate && !/^\d{8}$/.test(toDate))) {
    return res.status(400).json(
      buildError({
        message: "fromDate/toDate는 YYYYMMDD 형식이어야 합니다.",
        source: "SYSTEM",
        requestId,
        startedAt,
        errorCategory: ApiErrorCategory.VALIDATION_ERROR,
